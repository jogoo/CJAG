...
{
	const char *colors[] =
			{ [RED] = ANSI_COLOR_RED, [GREEN] = ANSI_COLOR_GREEN, [BLUE] = ANSI_COLOR_BLUE, [CYAN] = ANSI_COLOR_CYAN,
					[MAGENTA] = ANSI_COLOR_MAGENTA, [YELLOW] = ANSI_COLOR_YELLOW, [WHITE] = ANSI_COLOR_WHITE, [RESET
							] = ANSI_COLOR_RESET };
...
void printf_color(int enable, const char *fmt, ...)
{
	...
	for (i = 0; i < len; i++)
	{
		if (fmt[i] == '[' && ((i + 2 < len && fmt[i + 2] == ']') || (i + 3 < len && fmt[i + 1] == '/' && fmt[i + 3] == ']')))
		{
			if (i != len - 1)
			{
				if (fmt[i + 1] == '/')
